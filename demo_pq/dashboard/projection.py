...
class DashboardProjection:
    ...
    def update(self, entry: QueueEntry) -> None:
        ...
        row = {
            ...
            "priority_label": payload.get("priority_label", "normal"),
    ...
    def _sorted_rows(self) -> list[dict[str, Any]]:
        """Order highest priority first, then arrival order within a tier --
        the same order the priority queue itself dispatches in, so the
        table visibly shows what "priority" means rather than just arrival
        order with a coloured label attached."""
        return sorted(
            (row.copy() for row in self._rows.values()),
            key=lambda row: (-row["priority"], row["queued_at"], row["id"]),
        )
