...
Define the opt-in ASGI lifespan integration for a process-local generic queue
worker.
...
`runqueues` worker.
The local enqueue observation SHALL await the backend when testing for pending
entries rather than dispatching that test to a thread.
