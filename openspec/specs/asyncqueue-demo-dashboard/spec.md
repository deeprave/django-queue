# AsyncQueue Demo Dashboard

## Purpose

Define the minimal Django dashboard that visualises retained demo entries.
...
from the `demo` queue, including their identifier, state, queued, started, and
finished times.
...
The dashboard SHALL create a queue observer for `demo` when its local dashboard
receives its first page or event-stream request. It SHALL use lifecycle
snapshots to add or update displayed entry rows without altering the queue entry
itself. A terminal lifecycle snapshot SHALL update its corresponding displayed
row with the terminal state and finished time. An observer-only `terminated`
snapshot SHALL remove its corresponding row.
...
#### Scenario: Receive a termination lifecycle update
- **WHEN** the demo queue observer receives an observer-only `terminated`
  snapshot for a displayed entry
- **THEN** the dashboard removes that entry's row
...
exposes it on host port 16379. It SHALL make the dashboard available through an
