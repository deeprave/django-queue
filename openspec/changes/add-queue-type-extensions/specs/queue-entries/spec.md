...
representation. A backend MUST NOT instantiate an entry during settings or
queue construction; it SHALL do so only for enqueue, restore, or lifecycle
operations that require an entry value.
