...
- [ ] 6.1 Document the budget, its resolution order, the `TIMEOUT` setting, the
  `timeout` enqueue keyword, the heartbeat call, and the `mark_timed_out`
  backend contract addition in the README.
- [ ] 6.2 Run Ruff, ty, the full pytest suite repeatedly to confirm the new
