...
- [ ] 1.1 Add failing tests for the dual surface on `BaseQueue`: an `a`-prefixed
...
- [ ] 1.2 Make `BaseQueue` declare the asynchronous entry contract as its
...
- [ ] 1.3 Confirm the bridge's own error is the sync-called-from-async guard and
  assert on it directly; add no package exception and no custom check.

## 2. Memory backends

- [ ] 2.1 Add failing tests for the memory entry lifecycle through the
...
- [ ] 2.2 Convert the three memory backends to the asynchronous contract, with
...
- [ ] 2.3 Bridge `apoll` with `sync_to_async(thread_sensitive=False)`, since
  `queue.Queue.get(block=True)` blocks by design, and add a test that awaiting
  it does not stall other tasks on the loop.

## 3. Redis backends

- [ ] 3.1 Add failing tests for the Redis entry lifecycle through the
  asynchronous methods, covering enqueue, dequeue, each terminal transition, and
  the budget persisted on the record.
- [ ] 3.2 Move both Redis backends to `redis.asyncio.Redis`, awaiting every
  command, including the pipeline and Lua paths.
- [ ] 3.3 Acquire the client lazily keyed by the running event loop rather than
  ...
  operate correctly.
- [ ] 3.4 Implement `aclose` to release the resources belonging to the disposing
  loop, and add a test that a closed queue can be used again.
- [ ] 3.5 Confirm `RedisQueueClock` still calibrates against the asynchronous
...
- [ ] 4.1 Add a failing test that a complete dispatch, from dequeue through the
  terminal outcome, runs entirely on the event loop's own thread.
- [ ] 4.2 Remove the five `asyncio.to_thread` calls in `django_queue/worker.py`
  -- the dequeue, the mark-running, and the three in `_record_terminal` -- and
  await the backend instead.
- [ ] 4.3 Remove the `asyncio.to_thread` call in `django_queue/asgi.py` and
  await `ahas_pending_entries`.
- [ ] 4.4 Remove the `asyncio.to_thread` call in `runqueues` and await the same.
- [ ] 4.5 Assert no `asyncio.to_thread` remains anywhere in `django_queue/`.

## 5. Registry disposal

- [ ] 5.1 Add failing tests for asynchronous registry disposal and for the
  shutdown signal receiver staying a synchronous callable.
- [ ] 5.2 Add asynchronous disposal to the queue handler and have `close_queues`
  delegate through the bridge.

## 6. Heartbeat

- [ ] 6.1 Add failing tests for the heartbeat: extending from the handler
...
- [ ] 6.2 Publish the active dispatch's `asyncio.Timeout` in a `ContextVar` and
...
- [ ] 6.3 Add a test that a heartbeat does not alter the entry's recorded
  `ran_for`, since the budget and the entry's wall-clock timings stay
  independent.

## 7. Documentation and validation

- [ ] 7.1 Document the asynchronous surface in the README: the `a`-prefixed
...
- [ ] 7.2 Document the heartbeat as an assertion of progress a handler makes as
  it approaches its budget, and state plainly that it extends the budget only
  and is not an ownership or liveness guarantee.
- [ ] 7.3 Record in `add-redis-lease-recovery` that it must extend the heartbeat
...
- [ ] 7.4 Run Ruff, ty, the full pytest suite repeatedly to confirm the
